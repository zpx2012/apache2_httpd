...
static const char *set_timeout(cmd_parms *cmd, void *dummy, const char *arg)
{
    ...
    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, cmd->server, APLOGNO(00119), "set_timeout: set timeout to %d", cmd->server->timeout);
    return NULL;
}
...
static int core_pre_connection(conn_rec *c, void *csd)
{
    ...
    ap_log_cerror(APLOG_MARK, APLOG_DEBUG, rv, c, APLOGNO(00140)
                "core_pre_connection: set timeout to %d\n", c->base_server->timeout);
    if (rv != APR_SUCCESS) {
        /* expected cause is that the client disconnected already */
        ap_log_cerror(APLOG_MARK, APLOG_DEBUG, rv, c, APLOGNO(00140)
