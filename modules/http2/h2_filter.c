    ...
    if (APR_BRIGADE_EMPTY(cin->bb)) {
        ...
        if (block == APR_BLOCK_READ) {
            if (cin->timeout > 0) {
                ...
                ap_log_cerror(APLOG_MARK, APLOG_TRACE1, 0, f->c,"h2_filter_core_input: set timeout %d", cin->timeout);
            }
        }
        ...
        if (saved_timeout != UNSET) {
            ...
            ap_log_cerror(APLOG_MARK, APLOG_TRACE1, 0, f->c,"h2_filter_core_input: set timeout %d", saved_timeout);
        }
    }
