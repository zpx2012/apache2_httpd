...
static apr_status_t h2_session_send(h2_session *session)
{
    ...
    if (socket) {
        ...
        ap_log_cerror(APLOG_MARK, APLOG_TRACE1, 0, session->c,"h2_session_send(1327): set timeout %d", session->s->timeout);
    }
    ...
    if (socket) {
        ...
        ap_log_cerror(APLOG_MARK, APLOG_TRACE1, 0, session->c,"h2_session_send(1334): set timeout %d", saved_timeout);
    }
